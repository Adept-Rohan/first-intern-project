...
const Header: FunctionComponent = () => {
  ...
  // const data: RegisterData = useSelector((user: any) => user.user);
  // console.log("HEADER", data);

  const data = useSelector((user: any) => user.user.user);
  console.log("HEADER", data);
        ...
        <div className="flex items-center gap-8">
          <div className="relative flex items-center gap-2">
            ...
            <button onClick={() => setMenu(!menu)}>
              {data ? <div>{data.email}</div> : <div>Register</div>}
            </button>

            {menu ? (
              <div className="z-50 w-40 h-18 p-3 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-12  right-5">
                <Link to="/signin">
